...
const UrgentBanner = () => {
  ...
  useEffect(() => {
    const fetchUrgentCampaign = async () => {
      ...
      const response = await fetch('http://localhost:5000/api/campaign/all?status=active&sortBy=createdAt&sortOrder=desc&limit=50');
        ...
        const latestUrgentCampaign = data.campaigns?.find(campaign => campaign.isUrgent);
